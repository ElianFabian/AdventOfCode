    ...
    override fun getResultOfPartOne(): Int
    {
        ...
        instructions.forEach()
        {
            grid.setValueInSquare(
                topLeftCorner = it.topLeftCorner,
                bottomRightCorner = it.bottomRightCorner,
...
private fun <T> Matrix2D<T>.setValueInSquare(
    topLeftCorner: Vector2,
    bottomRightCorner: Vector2,
    getNewValue: (oldValue: T) -> T,
)
{
    for (rowIndex in topLeftCorner.x..bottomRightCorner.x)
    {
        for (columnIndex in topLeftCorner.y..bottomRightCorner.y)
        {
            val oldValue = this[rowIndex, columnIndex]

            this[rowIndex, columnIndex] = getNewValue(oldValue)
        }
    }
}
...
private fun fromLineToInstructionInfo(line: String): InstructionInfo
{
    ...
    return result.destructured.let { (action, topLeftX, topLeftY, bottomRightX, bottomRightY) ->
        ...
        InstructionInfo(
            ...
            topLeftCorner = Vector2(topLeftX.toInt(), topLeftY.toInt()),
            bottomRightCorner = Vector2(bottomRightX.toInt(), bottomRightY.toInt()),
        )
    }
}

private data class InstructionInfo(
    val topLeftCorner: Vector2,
    val bottomRightCorner: Vector2,
    val action: Action,
)
...
private val instructionRegex = "(\\w+|\\w+ \\w+) (\\d+),(\\d+) through (\\d+),(\\d+)".toRegex()
