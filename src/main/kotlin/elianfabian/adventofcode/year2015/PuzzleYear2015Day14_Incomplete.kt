...
object PuzzleYear2015Day14_Incomplete : AocPuzzle(2015, 14)
{
    ...
    override fun getResultOfPartOne(): Int
    {
        ...
        val info = input.lines().map { fromLineToReindeerRaceInfo(it) }

        val totalDistanceByReindeerName = info.associate { it.reindeerName to it.getTotalDistanceInKmTraveled(totalTimeInSeconds = raceTimeInSeconds) }
    ...
    override fun getResultOfPartTwo(): Int
    {
        ...
        val infoByReindeerName = input.lines().associate()
        {
            val info = fromLineToReindeerRaceInfo(it)
