...
object PuzzleYear2015Day3 : AocPuzzle(2015, 3) {
	...
	override fun getResultOfPartOne(): Int {
		...
		val allLocationsThatSantaHasBeen = allMovements.runningFold(initialLocation) { currentLocation, direction ->
			currentLocation + direction
		}.distinct()

		return allLocationsThatSantaHasBeen.size
	}

	override val partTwoQuestion = "This year, how many houses receive at least one present?"
	...
	override fun getResultOfPartTwo(): Int {
		...
		val allLocationsThatSantaHasBeen = santaMovements.runningFold(initialLocation) { currentLocation, direction ->
			currentLocation + direction
		}.distinct()

		val allLocationsThatRoboSantaHasBeen = roboSantaMovements.runningFold(initialLocation) { currentLocation, direction ->
			currentLocation + direction
		}.distinct()

		return (allLocationsThatSantaHasBeen + allLocationsThatRoboSantaHasBeen).distinct().size
	}
